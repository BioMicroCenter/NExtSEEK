...
def main() -> int:
    ...
    for mutant in selected_mutants:
        ...
        with tempfile.TemporaryDirectory(prefix=f"attribute-{mutant_id}-") as temporary:
            ...
            for phase in ("original", "mutated", "restored"):
                pytest_report = root / f"mutant-{mutant_id}-{phase}-pytest.json"
                env = dict(os.environ, ATTRIBUTE_ACTIVE_MUTANT_ID=mutant_id,
                           ATTRIBUTE_MUTANT_PROOF_PHASE=phase,
                ...
                try:
                    phase_runs[phase] = subprocess.run(
        ...
        report_paths = {phase: root / f"mutant-{mutant_id}-{phase}-pytest.json"
                        for phase in ("original", "mutated", "restored")}
        if not handshake.is_file() or any(not path.is_file() for path in report_paths.values()):
        ...
        if (set(proof) != HANDSHAKE_KEYS or proof["mutant_id"] != mutant_id
            ...
            result["errored"].append(mutant_id); continue
        reports = {phase: json.loads(path.read_text()) for phase, path in report_paths.items()}
        valid_report = all(
            set(report) == REPORT_KEYS
            and report["proof_phase"] == phase
            and report["killer"] == killer
            and report["pytest_exit_code"] == phase_runs[phase].returncode
        ...
        clean_phases = all(
            phase_runs[phase].returncode == 0
            and all(row["outcome"] == "passed" for row in reports[phase]["phases"] if row["phase"] == "call")
            for phase in ("original", "restored")
        )
        if (not valid_report or not clean_phases or infrastructure_failures
                or phase_runs["mutated"].returncode not in {0, 1}):
            result["errored"].append(mutant_id)
        elif assertion_failures and phase_runs["mutated"].returncode == 1:
            result["killed"].append(mutant_id)
        elif phase_runs["mutated"].returncode == 0:
    ...
    reports = {
        row["id"]: {
            phase: json.loads((root / f"mutant-{row['id']}-{phase}-pytest.json").read_text())
            for phase in ("original", "mutated", "restored")
        }
