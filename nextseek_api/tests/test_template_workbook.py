...
def test_the_readme_carries_no_provenance_tree(tmp_path, _no_lookups):
    """A blank template has no provenance to draw.

    Sample downloads render an indented derivation tree into the README; a
    template has no samples, so it must pass no flow lines and the heading must
    not appear. This replaced an assertion about a separate flow *sheet*, which
    upstream folded into the README.
    """
    from nextseek_api.services.sample_workbook import FLOW_HEADING, README_SHEET
