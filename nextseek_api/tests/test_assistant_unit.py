...
# Base patches for all pipeline tests
_PIPELINE_BASE = "nextseek_api.assistant.pipeline_adapter"
...
class PipelineAdapterTests(TestCase):
    ...
    @patch(f"{_PIPELINE_BASE}.shortlist_catalog")
    @patch(f"{_PIPELINE_BASE}.entity_agent")
    @patch(f"{_PIPELINE_BASE}.parser_agent")
    @patch(f"{_PIPELINE_BASE}.fix_sample_endpoint", side_effect=lambda d: d)
    @patch(f"{_PIPELINE_BASE}.api_agent_build_request")
    @patch(f"{_PIPELINE_BASE}.tool_nextseek_api_request")
    @patch(f"{_PIPELINE_BASE}._retry_advanced_search_if_empty")
    @patch(f"{_PIPELINE_BASE}.normalize_api_result_for_memory", return_value={})
    @patch(f"{_PIPELINE_BASE}.slim_api_result_for_llm", return_value={})
    @patch(f"{_PIPELINE_BASE}.chatter_agent_search_answer", return_value="Found 5 samples.")
    @patch(f"{_PIPELINE_BASE}.get_schema_for_endpoint", return_value=None)
    ...
    @patch(f"{_PIPELINE_BASE}.parser_agent")
    @patch(f"{_PIPELINE_BASE}.fix_sample_endpoint", side_effect=lambda d: d)
    @patch(f"{_PIPELINE_BASE}.memory_agent_answer", return_value="There are 3 monkeys.")
    ...
    @patch(f"{_PIPELINE_BASE}.shortlist_catalog", side_effect=Exception("boom"))
    ...
    @patch(f"{_PIPELINE_BASE}.normalize_api_result_for_memory", return_value={})
    @patch(f"{_PIPELINE_BASE}.slim_api_result_for_llm", return_value={})
    @patch(f"{_PIPELINE_BASE}.chatter_agent_search_answer", return_value="Refined results.")
    @patch(f"{_PIPELINE_BASE}.get_schema_for_endpoint", return_value=None)
    ...
    @patch(f"{_PIPELINE_BASE}.parser_agent")
    @patch(f"{_PIPELINE_BASE}.fix_sample_endpoint", side_effect=lambda d: d)
    @patch(f"{_PIPELINE_BASE}.reporter_agent")
    @patch("chat_nextseek.helpers.run_project_sample_report")
    @patch(f"{_PIPELINE_BASE}.persist_report_file", return_value=None)
    @patch(f"{_PIPELINE_BASE}.top_items", return_value=[])
    @patch(f"{_PIPELINE_BASE}.chatter_agent_report_answer", return_value="Report summary.")
    ...
    @patch(f"{_PIPELINE_BASE}.fix_sample_endpoint", side_effect=lambda d: d)
    @patch(f"{_PIPELINE_BASE}.reporter_agent")
    @patch(f"{_PIPELINE_BASE}.generate_report_outputs")
...
class ViewSetTests(TestCase):
    ...
    def test_test_cases_admin(self):
        ...
        self.assertEqual(len(resp.data["test_cases"]), 15)
