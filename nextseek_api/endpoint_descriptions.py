...
BATCH_UPLOAD_START_DESC = (
    ...
    "**EXAMPLES:**\n"
    "- 'Upload this Excel file to create samples in bulk for project 4475'\n"
    "- 'Start a batch upload from /data/samples.xlsx'\n"
    "- 'Bulk import samples from the spreadsheet'\n\n"
    "---\n\n"
    ...
    "- **sop_id** — Optional SOP id override. If provided and `json_metadata.Protocol` contains "
    "an SOP id, they must match or the row is rejected.\n"
    "- **assay_titles** — Optional assay title(s). If provided, parsed into a list and must match "
    "`assay_ids` length (or both singleton) or the row is rejected.\n\n"
    "## Notes\n\n"
    "- Column headers are case-insensitive and whitespace is trimmed.\n"
    "- Unknown extra columns are ignored, but a warning listing them is included in logs, "
    "the job status payload, and the summary CSV.\n"
    "- Some validation conflicts (e.g., UID mismatch between `uid` and `json_metadata.UID`) "
    "reject only that row; the job continues.\n"
)
