...
GRAPH_SEARCH_DESC = (
    ...
    "`<=`, `>`, `>=`, `IN` (a list value), `CONTAINS`, `NOT CONTAINS`, `STARTS WITH`, exact and case-sensitive, the "
    "value cast by the attribute's type (the string operators compare the stored value's text; `NOT CONTAINS` keeps "
    "only samples that hold the attribute), or `IS TRUE` / `IS FALSE` with no value (true is a boolean true, 1, or "
    "the text `1`, `true` or `yes`; false is any other value the sample holds); and `lineage`, `{direction: ancestor or descendant, sample_type, max_hops: 1 to 4}`. Query "
