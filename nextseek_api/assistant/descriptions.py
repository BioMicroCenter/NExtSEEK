...
ASSISTANT_SESSION_DOWNLOAD_DESC = (
    ...
    "from inside the outputs roots; Container-CC files only from the owner's CC tree, the project folder "
    "the chat's CC turns ran in, so a superuser's download of another user's chat carries them too. "
    "`manifest.json` names every file shipped and every file left out, with a reason code, never a "
    "server path.\n\n"
