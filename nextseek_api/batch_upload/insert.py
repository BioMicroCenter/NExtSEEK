...
def process_batches(
    ...
    while cursor < len(rows_to_process):
        ...
        batch, next_cursor, _payload = _plan_next_batch(
            rows_to_process, cursor, sizer.current_size, config.max_payload_bytes
        )
        cursor = next_cursor
        ...
        try:
            with conn_factory() as conn:
                ...
                uid_policy_pairs = insert_policies_for_uids(
                    batch_uids, contributor_id, conn
                )
                ...
                for sample in batch:
                    rows_payload.append({
                        ...
                        "uuid": sample.uuid,
                        "contributor_id": contributor_id,
                        "first_letter": compute_first_letter(sample.title),
                ...
                for sample in batch:
                    ...
                    if sid and sample.assay_ids:
                        assay_records.append((sample.uuid, sid, sample.assay_ids))
                assay_link_count = batch_insert_assay_assets(
                    assay_records, direction_by_pair, conn
                )
