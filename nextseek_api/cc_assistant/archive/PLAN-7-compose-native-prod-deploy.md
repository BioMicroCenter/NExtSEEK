...
- **Spec of record:** `nextseek_api/cc_assistant/archive/SPEC-7-compose-native-prod-deploy.md`
...
- **nextseek_api/cc_assistant/DEPLOY.md merge order:** PLAN-3 Task 13 may append Step-3 deploy notes first. Step 7 Task 8 rewrites Phase A/B only on the **post-Step-3-deployed** commit hash recorded in preflight — never on stale pre-deploy docs.
- **Coverage targets (Phase 2 hardened):** Hermetic implementation modules run `--cov=nextseek_api.cc_assistant.tests.validate_step7_compose_deploy --cov-fail-under=95`. **Task 16 correction (final-review debt fix):** run exactly as written above, this command is **silently defeated** — the repo's `.coveragerc` (`[run] omit = ... nextseek_api/*/tests/*`) excludes both `validate_step7_compose_deploy.py` and `step7_preflight_collector.py` from measurement (they live under `nextseek_api/cc_assistant/tests/`), so `coverage`/`pytest-cov` silently reports **no data collected** rather than gating anything. The canonical, non-defeated invocation passes an **explicit neutral `--rcfile`** so the repo `.coveragerc` omit never applies:
  ```
  python -m coverage run --rcfile=/dev/null --source=nextseek_api.cc_assistant.tests.validate_step7_compose_deploy \
    -m pytest -q nextseek_api/cc_assistant/tests/ --ignore=nextseek_api/cc_assistant/tests/test_cc_realstack.py
  python -m coverage report --rcfile=/dev/null --fail-under=95
  ```
  (swap `--source=` for `...step7_preflight_collector` to gate that module too; both are held to the same ≥95% floor as validator modules under Global Constraints — see `.superpowers/sdd/task-16-report.md` for the measured before/after numbers.) The **behavior-bearing Task 6 mount refactor is covered explicitly** (not left to the validator floor), but **rescoped to the pure code Task 6 actually adds/edits**: `cc_engine.py` is **not** whole-module gated because it is a pre-existing *procedural* module that also holds the **live-only `run_cc_turn`** (`cc_engine.py:398–607` — ~31% of the 672-line module; called **only** by the live `test_cc_realstack.py`, never hermetically) and the `cc_runner_available` live `client.networks.get` branch, so a whole-module `--cov=cc_engine --cov-fail-under=95` is unreachable within this task's surgical scope (same situation as PLAN-3 Task 5 `translate`). It splits into three honest pieces:
...
Also record: Step 3 doc commit state, compose services, top-level compose networks, CC keys in `docker/nextseek.env.example`, whether `nextseek_api/cc_assistant/DEPLOY.md` still contains old manual bootstrap commands, `docker version` / `docker info` summary, **`docker compose version`** (Compose ≥2.26 if compose YAML uses volume subpaths), and Task 6 subpath floor (`docker_engine_meets_subpath_floor`).

- [ ] **Success conditions**

- A generated `preflight.json` exists and is valid JSON.
- It includes hashes for `docker-compose.yml`, `docker/nextseek.env.example`, `nextseek_api/cc_assistant/DEPLOY.md`, `SPEC-3-ui-based-io.md`, and `PLAN-3-ui-based-io.md` if present.
...
- `nextseek_api/cc_assistant/DEPLOY.md` names only gitignored files for secrets.
- Old manual sidecar bootstrap is retired from the required path.
- Top-level docs point to `nextseek_api/cc_assistant/DEPLOY.md` instead of duplicating stale instructions.
...
| 6 | 5/8 | nextseek_api/cc_assistant/DEPLOY.md merge conflict with PLAN-3 Task 13 | Operators get contradictory procedures | Merge order: Step 3 deploy first, then Step 8 rewrite on recorded hash |
...
| PLAN-3 overlap (`nextseek_api/cc_assistant/DEPLOY.md`, `cc_config.py`, `cc_engine.py`) | Step 3 not started at vetting baseline | Sequencing gate enforced |
...
> template), Task 8 (nextseek_api/cc_assistant/DEPLOY.md), and Tasks 9-10 (evidence gates) are executed, so 7b/7c/7d
...
- The sidecar's staging mount is the RESERVED top-level `_staging/` subtree of `dmac-cc-users`
  ...
  **Dedicated-volume fallback blast radius (pre-enumerated, iter-1 M-1):**
  ...
  greenfield oracle, Task 10 Step 0's "seven `external: true` volumes" wording, `nextseek_api/cc_assistant/DEPLOY.md`
