...
if mode == "seed"
  ...
  tokens = {}
  revoked_token = nil
  # Rails' universal ApplicationRecord authorization-enforcement callback requires an
  # authenticated actor context that a headless `rails runner` seed process never has;
  # disabling it here only bypasses the seeding write path, never the oracle's read of
  # Person#is_admin?, which is the sole value this fixture supplies to the acceptance test.
  disable_authorization_checks do
    ActiveRecord::Base.transaction do
      ...
      # SEEK auto-grants the first created account a persisted system-admin role during
      # User.create!. Strip it so ordinary-user remains a true non-admin negative case.
      ordinary = Person.joins(:user).find_by!(users: {login: "ordinary-user"})
      Role.where(:person_id => ordinary.id, :role_type_id => 1).delete_all
