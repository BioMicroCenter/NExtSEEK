...
- **The write gate knows eight labels; the dispatch table has ten.** `SIDECAR_OPS` in `write_gate.py` holds the seven ported labels plus `graph-schema`. `run-ls` and `build-upload-xlsx` pass only because they never call the gate (only the `api-read` and `api-write` handlers do; see `nextseek_api/assistant/CONTRACT.md`).
