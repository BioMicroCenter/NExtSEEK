...
def test_local_staging_copies_cohort_json_when_it_exists_beside_the_sheet(tmp_path):
    # emit_launch_artifacts' local-fs staging path (SEQERA_INPUT_DIR set) co-stages
    # cohort.json beside the staged samplesheet.csv, best-effort. Use an explicit
    # input_dir so the staged copy lands under tmp_path rather than a real /bucket.
    sheet = tmp_path / "samplesheet.csv"
    sheet.write_text("sample,accession\nS1,SRR1\n", encoding="utf-8")
    (tmp_path / "cohort.json").write_text(
        json.dumps([{"d_seq_uid": "D.SEQ-1", "nfcore_sample": "S1"}]), encoding="utf-8")
    input_dir = tmp_path / "staged"
    tower_env = dict(TOWER, input_dir=str(input_dir))
    emit_launch_artifacts(
        tmp_path, pipeline="rnaseq", samplesheet_path=sheet,
        launch_plan={"run_name": "r1", "params": {}}, tower_env=tower_env, excluded=[])
    staged_cohort = input_dir / "r1" / "cohort.json"
    assert staged_cohort.read_text() == (tmp_path / "cohort.json").read_text()


def test_local_staging_without_a_cohort_json_does_not_error(tmp_path):
    # No cohort.json beside the source samplesheet (e.g. emit_launch_artifacts called
    # standalone, as in test_emit_launch_artifacts_alone_writes_yamls) must not raise.
    sheet = tmp_path / "samplesheet.csv"
    sheet.write_text("sample,accession\nS1,SRR1\n", encoding="utf-8")
    input_dir = tmp_path / "staged2"
    tower_env = dict(TOWER, input_dir=str(input_dir))
    res = emit_launch_artifacts(
        tmp_path, pipeline="rnaseq", samplesheet_path=sheet,
        launch_plan={"run_name": "r1", "params": {}}, tower_env=tower_env, excluded=[])
    assert (input_dir / "r1" / "samplesheet.csv").exists()
    assert not (input_dir / "r1" / "cohort.json").exists()
    assert res.saved_files.get("staged_samplesheet")
