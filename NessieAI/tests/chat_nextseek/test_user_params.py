...
def test_pipelines_with_no_required_user_params_are_never_blocked():
    for key in ("rnaseq", "scrnaseq", "seqinspector"):
        assert required_user_params(key) == [], key
        assert missing_user_params(key, {}) == [], key


def test_hlatyping_hlahd_path_is_required_only_when_tools_is_hlahd():
    """hlatyping declares hlahd_path, but it is conditional: OptiType (the default)
    needs nothing, so a run that never selects HLA-HD is never blocked."""
    assert [s["name"] for s in required_user_params("hlatyping")] == ["hlahd_path"]
    # default tool (optitype) -> not required
    assert missing_user_params("hlatyping", {}) == []
    assert missing_user_params("hlatyping", {"tools": "optitype"}) == []
    # switch to HLA-HD -> hlahd_path becomes required
    missing = missing_user_params("hlatyping", {"tools": "hlahd"})
    assert [s["name"] for s in missing] == ["hlahd_path"]
