...
CHECK_CYPHER = (
    ...
    "RETURN c.uid AS uid, a IS NOT NULL AS exact, b.uuid AS base_uuid,\n"
    "       COLLECT { MATCH (p:Sample) WHERE p.uuid STARTS WITH c.base + '-PUB'\n"
    "                 RETURN p.uuid ORDER BY p.uuid LIMIT 2 } AS suffixed"
)
...
def check_uids(config: Any, uids: list[str], *, run: Callable[..., dict]) -> list[UidCheck] | None:
    """One read-only query; ``run`` is ``tool_neo4j_query``. None when the query failed."""
    if not uids:
        return []
...
def uid_notes(checks: list[UidCheck] | None) -> tuple[str | None, list[str]]:
    ...
    for check in checks:
        ...
        if check.stored:
            ...
            agent.append(
                f"UID {check.asked} is not in the graph, with or without a -PUB suffix. Do not describe it "
                "as a sample with no parents or no children: it was not found."
            )
            reply.append(
                f"{check.asked} was not found in the graph (checked with and without a -PUB suffix). Say "
                "plainly that this UID was not found; a zero in the result means the UID is absent, not "
                "that it has no parents or children."
            )
    if not agent:
        return None, []
