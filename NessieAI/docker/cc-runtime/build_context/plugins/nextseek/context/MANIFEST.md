...
| `min_api_endpoints_enriched.json`, `min_api_endpoints.json` | REST endpoints, methods, and parameters (enriched has descriptions). Sample search is not among the endpoints you call: every sample question goes to `nextseek-graph`. | Building a `nextseek-parse` plan or a `nextseek-api-read` body for a record or people request. No write reaches NExtSEEK from this chat. |
| **not a file: run `nextseek-graph-schema`** | The **deployed** graph's schema, read live: node labels and relationships, an index of every sample type (code, name, how many samples, how many attributes hold values), and the investigation and project titles. Pass `--types "TIS,D.SEQ"` for those types' attributes, their value types and how many samples fill each, with numeric and date bounds; pass `--query "<question>"` for the study, assay or protocol titles that question names. It never returns stored values. Its `source` field says `catalog` (the live graph) or `fallback` (a committed capture, with the reason): check it before trusting the answer. | When you need an attribute's exact title, its value type or its bounds to phrase a question, or the user asks what the graph holds. For the values an attribute holds, ask `nextseek-aggregate`. `nextseek-graph` reads the live catalog itself, so this is not a required step before every query. There is no baked graph-schema file to read: a capture goes stale the moment the graph is synced, so ask the graph. |
...
- **How many samples, broken down by what** (counts, breakdowns by type / attribute / keyword / assay / project / person, tallies, histograms, which values an attribute holds and how often, variant spellings): `nextseek-aggregate`, one call, with one `--parts` entry per independent number. It returns small tables, never sample records, and is held to the user's projects the same way. A breakdown's `sum_of_group_counts` counts a sample once per group it falls in, so it is not a number of samples when `groups_may_overlap` is true.
