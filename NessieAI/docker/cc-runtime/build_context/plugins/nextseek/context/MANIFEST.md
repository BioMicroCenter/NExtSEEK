...
- **A record, people, a file, one sample's full export, or a write**: `nextseek-parse` → `nextseek-api-read` / `nextseek-api-write`. `/people/` lists registered users; the person on a sample is its `Scientist` attribute, a graph question.
