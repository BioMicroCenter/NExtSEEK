...
- Nothing here generates or compares `min_graph_schema.json`, although it is baked into the plugin context. Its drift, which does reach the agent, is recorded in `NessieAI/docker/CLAUDE.md`. `neo4j_schema.json` is no longer baked: the agent reads the live graph through the `nextseek-graph-schema` op.
