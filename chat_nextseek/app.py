...
with st.sidebar.expander("🧪 Scripted tests", expanded=False):
    suites: dict[str, list[dict]] = {}
    if isinstance(TEST_CASES, dict):
        for suite_name, cases in TEST_CASES.items():
            suite_list: list[dict] = []
            for case_id, payload in cases.items():
                entry = {"id": case_id, "suite": suite_name}
                if isinstance(payload, dict):
                    entry.update(payload)
                suite_list.append(entry)
            suites[suite_name] = suite_list
        loaded_cases = [case for suite in suites.values() for case in suite]
    elif isinstance(TEST_CASES, list):
        loaded_cases = TEST_CASES
    else:
        st.warning("TEST_CASES is not a list or dict. Check test.py.")
        loaded_cases = []

    st.write(f"Loaded {len(loaded_cases)} cases from test.py")

    if "scripted_test_results" not in st.session_state:
        st.session_state["scripted_test_results"] = []

    selected_suite = None
    if suites:
        suite_names = ["<run all>"] + list(suites.keys())
        selected_suite = st.selectbox(
            label="Select test suite to run",
            options=suite_names,
            key="suite_run_select",
        )

    run_tests = st.button(
        "Run scripted tests",
        disabled=not loaded_cases,
        type="primary",
        use_container_width=True,
    )

    if run_tests and loaded_cases:
        results = []
        cases_to_run = loaded_cases
        if suites and selected_suite:
            if selected_suite == "<run all>":
                cases_to_run = loaded_cases
            else:
                cases_to_run = suites.get(selected_suite, [])
        for idx, case in enumerate(cases_to_run, start=1):
            prompt = case.get("prompt") or case.get("question") or case.get("text")
            case_id = case.get("id") or f"case_{idx}"
            if not prompt:
                results.append({"id": case_id, "ok": False, "error": "Missing 'prompt' field"})
                continue
            with st.status(f"Running {case_id}", expanded=True) as status:
                status.write(f"Prompt: {prompt}")
                try:
                    result = run_query(st.session_state, config, prompt)
                    reply = result["reply"]
                    results.append({"id": case_id, "ok": True, "prompt": prompt, "reply": reply})
                    status.update(label=f"Finished {case_id}", state="complete")
                except Exception as e:
                    results.append({"id": case_id, "ok": False, "prompt": prompt, "error": repr(e)})
                    status.update(label=f"Failed {case_id}", state="error")
        ...
        st.session_state["scripted_test_results"] = results

    if loaded_cases:
        st.divider()
        st.caption("Send a scripted case prompt to the main chat input:")
        if suites:
            for suite_name, suite_cases in suites.items():
                st.write(f"{suite_name}")
                case_options = [c.get("id") or "case" for c in suite_cases]
                selected = st.selectbox(
                    label=f"{suite_name} case",
                    options=case_options,
                    key=f"suite_select_{suite_name}",
                    label_visibility="collapsed",
                )
                chosen = None
                for case in suite_cases:
                    if case.get("id") == selected:
                        ...
                        break
                if chosen:
                    prompt = chosen.get("prompt") or chosen.get("question") or chosen.get("text")
                    disabled = not prompt
                    if st.button(
                        f"Prefill {selected}",
                        key=f"prefill_{suite_name}_{selected}",
                        disabled=disabled,
                        use_container_width=True,
                    ):
                        st.session_state[CHAT_PREFILL_KEY] = prompt or ""
                        st.rerun()
        else:
            for idx, case in enumerate(loaded_cases, start=1):
                prompt = case.get("prompt") or case.get("question") or case.get("text")
                case_id = case.get("id") or f"case_{idx}"
                disabled = not prompt
                bullet, btn_col = st.columns([0.08, 0.92])
                with bullet:
                    st.markdown("•")
                with btn_col:
                    if st.button(
                        case_id,
                        key=f"prefill_{case_id}_{idx}",
                        disabled=disabled,
                        use_container_width=True,
                    ):
                        # Request a prefill; applied before chat_input renders on rerun.
                        st.session_state[CHAT_PREFILL_KEY] = prompt or ""
                        st.rerun()

    if st.session_state.get("scripted_test_results"):
        st.write("Last scripted run:")
        for r in st.session_state["scripted_test_results"]:
            st.write(r)
