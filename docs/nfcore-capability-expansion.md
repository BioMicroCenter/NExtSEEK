...
## 4. Rules Claude needed to learn
...
**Where to start.** `detaxizer` is done and `ampliseq` is part-way (both
2026-08-05). Finish `ampliseq` first — it covers our biggest library
type and needs only real primer sequences to complete. Then `crisprseq`,
to prove the question-asking works end to end. Leave `hic` and `rnavar`
for last, since they build a large index first and a failure takes hours
to surface.
