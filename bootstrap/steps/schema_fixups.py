...
class MissingColumn:
    ...
    table: str
    column: str
    column_definition: str               # e.g. "JSON NULL" — the suffix after ADD COLUMN <name>
...
def _apply_fixup(fix: MissingColumn, repo_root: Path, env: dict[str, str]) -> None:
...
def apply_all(repo_root: Path, env: dict[str, str]) -> list[tuple[str, str]]:
    ...
    Status is one of:
      - "applied"          — column was missing, ALTER + backfill ran
      - "already present"  — column existed; no-op
      - "table missing"    — target table absent (e.g., --no-seed install); fixup skipped
    """
    ...
    for fix in KNOWN_FIXUPS:
        ...
        if not _table_exists(fix, repo_root, env):
            ...
            continue
        if _column_exists(fix, repo_root, env):
            results.append((fqn, "already present"))
            continue
        _apply_fixup(fix, repo_root, env)
