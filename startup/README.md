...
`startup/seed/sql/` ships eleven `CREATE TABLE IF NOT EXISTS` files, five of which are
...
seed. The three `*.curated.sql` files are generated by `scripts/context_gen.py` and held:
no install step reads them until their content is signed off (`scripts/README.md` group C). `startup/templates/` holds the three files rendered into `docker/db.env`,
